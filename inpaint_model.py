...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, config=None, reuse=False,
        ...
        with tf.variable_scope(name, reuse=reuse), \
            ...
            # 并联跨步卷积 start
            x_first = x
            x = gen_conv(x_first, 4*cnum, 3, rate=2, name='conv7_atrous_top')
            x = gen_conv(x, 4*cnum, 3, rate=4, name='conv8_atrous_top')
            x = gen_conv(x, 4*cnum, 3, rate=8, name='conv9_atrous_top')
            x = gen_conv(x, 4*cnum, 3, rate=16, name='conv10_atrous_top')
            first_top = x
            x = gen_conv(x_first, 4*cnum, 3, rate=3, name='conv7_atrous_bottom')
            x = gen_conv(x, 4*cnum, 3, rate=6, name='conv8_atrous_bottom')
            x = gen_conv(x, 4*cnum, 3, rate=12, name='conv9_atrous_bottom')
            x = gen_conv(x, 4*cnum, 3, rate=24, name='conv10_atrous_bottom')
            first_bottom = x
