...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, config=None, reuse=False,
        ...
        with tf.variable_scope(name, reuse=reuse), \
            ...
            x = gen_conv(x, 4*cnum, 3, rate=3, name='conv8_atrous_top')
            x = gen_conv(x, 4*cnum, 3, rate=5, name='conv9_atrous_top')
            x = gen_conv(x, 4*cnum, 3, rate=7, name='conv10_atrous_top')
            first_top = x
